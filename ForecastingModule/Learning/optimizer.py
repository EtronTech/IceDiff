...
def build_optimizer(config, model, simmim=False, is_pretrain=False):
    ...
    if opt_lower == 'sgd':
        optimizer = optim.SGD(parameters, momentum=config.TRAIN.OPTIMIZER.MOMENTUM, nesterov=True,
                              lr=config.TRAIN.OPTIMIZER.BASE_LR, weight_decay=config.TRAIN.OPTIMIZER.WEIGHT_DECAY)
    elif opt_lower == 'adamw':
        optimizer = optim.AdamW(parameters, eps=config.TRAIN.OPTIMIZER.EPSILON, betas=(config.TRAIN.OPTIMIZER.BETAS[0],config.TRAIN.OPTIMIZER.BETAS[1]),
                                lr=config.TRAIN.OPTIMIZER.BASE_LR, weight_decay=config.TRAIN.OPTIMIZER.WEIGHT_DECAY)
...
def set_weight_decay(model, skip_list=(), skip_keywords=()):
    ...
    modules = [model.get_encoder, model.get_decoder]
...
def get_swin_layer(name, num_layers, depths):
    if name in ("mask_token"):
        ...
        return 0
    elif name.startswith("output"): # Decoder DePatchEmbd
        return 0
