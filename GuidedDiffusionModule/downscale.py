...
def main():
    ...
    # x_lr->y  x_in->x_0  x_in_lr->D(x_0)
    def general_cond_fn(x, t, weight=None, light_mask=None, guidance_scale = None, corner=None, scale=None, y=None, x_lr=None, sample_noisy_x_lr=False, diffusion=None, sample_noisy_x_lr_t_thred=None):
        ...
        with th.enable_grad():
            ...
            if not x_lr is None: 
                ...
                loss = loss - mse * args.img_guidance_scale # move xt toward the gradient direction 
                # loss = loss + guidance_scale * mse
                ...
                # light_mask = light_mask - th.autograd.grad(mse, light_mask,retain_graph=True)[0] * 0.1
    ...
    for i, data in enumerate(dataloader):
        # data[0] from dataloader, data[1] from dataloader_lr
        # traverse images
        if args.use_img_for_guidance:
            image, label = data[0]
            ...
            # 448 304
            image_size_h = image_lr.shape[2]
            image_size_w = image_lr.shape[3]
            cond_fn = lambda x,t,weight,light_mask,guidance_scale,corner,y : general_cond_fn(x, t, weight=weight,light_mask=light_mask,guidance_scale = guidance_scale, corner=corner, y=y, x_lr=image_lr, sample_noisy_x_lr=args.sample_noisy_x_lr, diffusion=diffusion, sample_noisy_x_lr_t_thred=args.sample_noisy_x_lr_t_thred)

        else:
            image, label = data
            cond_fn = lambda x,t,y : general_cond_fn(x, t, y=y, x_lr=None)
        if args.start_from_scratch:
            shape = (image.shape[0], 3, image_size_h, image_size_w)
        else:
        ...
        weight = th.rand([1,1,3,3], device=device)/100
        ...
        guidance_scale = torch.tensor(2000)
        sample_y = image_lr
        ...
        if args.save_png_files:
            save_images(sample, classes, start_idx + len(all_images) * args.batch_size, os.path.join(logger.get_dir(), 'images'),image_size_h,image_size_w,multiple)
        save_images(image_lr, label.long(), start_idx + len(all_images) * args.batch_size, os.path.join(logger.get_dir(), 'lr'),image_size_h,image_size_w,1)
        all_images.append(sample)
        all_labels.append(classes)
        logger.log(f"created {len(all_images) * args.batch_size} samples")

    print("average time", total_time/len(all_images))
...
def create_argparser():
    ...
    save_dir = '' # Default save dir
    base_samples = '' # Default GT/Forecasted SIC
    ...
    parser.add_argument("--img_guidance_scale", default=3800, type=float, help='guidance scale')
