            ...
            if cond_fn is not None:
                for i in range(1):
                    out["mean"], weight_tmp,light_mask_tmp,gradient,mse = self.condition_mean(
                        cond_fn, out, x[:, :, hi:hi + p_size, wi:wi + p_size], out["pred_xstart"], t, weight=weight,light_mask = light_mask[hi:hi + p_size, wi:wi + p_size],guidance_scale=guidance_scale, corner=[hi, wi, p_size],scale=scale, model_kwargs=model_kwargs
                    )
